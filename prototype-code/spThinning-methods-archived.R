...
accepted_name_v <- accepted_name_v[task_id] # index by task ID
...
part <- paste0("part", task_id, "-accepted_name_v")
#for(i in 1:length(accepted_name_v)){
  if(file.exists(paste0("./data/processed/coord-clean-data/", accepted_name_filestyle_v, ".csv"))){
    occur_data <- fread(paste0("./data/processed/coord-clean-data/", accepted_name_filestyle_v, ".csv"))
  } else{
    print(paste("No occur data for", accepted_name_v))
  ...
  occur_data <- occur_data[, name := accepted_name_v]
  ...
  spThin::thin(loc.data = occur_data, 
               ...
               thin.par = 1,
               max.files = 1,
               out.base = paste0(accepted_name_filestyle_v),
               reps = 100, 
  ...
  thinned_data_1km <- fread(paste0("./data/processed/sp-thinned-data/1km/", accepted_name_filestyle_v, "_thin1.csv"))
  num_occur_data_1km_post_spthin <- nrow(thinned_data_1km)
  thinned_data_5km <- fread(paste0("./data/processed/sp-thinned-data/5km/", accepted_name_filestyle_v, "_thin1.csv"))
  ...
  one_km_plot <- ggplot() + 
    ...
    theme_bw() +
    ggtitle(paste("1km Thinned Occurrence Data for", accepted_name_v)) +
  ...
  # Save as a jpg
  ggsave(one_km_plot, file = paste0("./outputs/sp-thin-plots/1km/", accepted_name_filestyle_v, "thinned.jpg"), height = 10, width = 10)
  ...
  five_km_plot <- ggplot() + 
    ...
    coord_sf(xlim = c(hull_bbox[1], hull_bbox[3]), ylim = c(hull_bbox[2], hull_bbox[4])) +
    theme_bw() +
    ggtitle(paste("5km Thinned Occurrence Data for", accepted_name_v)) +
    theme(plot.title=element_text(hjust = 0.5)) +
    xlab("Longitude") + 
    ylab("Latitude") + 
    scale_color_manual(name = "Thinning Status", 
  ...
  # Save as jpg
  ggsave(five_km_plot, file = paste0("./outputs/sp-thin-plots/5km/", accepted_name_filestyle_v, "thinned.jpg"), height = 10, width = 10)
  
  # Create info tb
  # create table 
  sp_thin_info_tb <- data.frame(
    name = accepted_name_v,
  ...
  fwrite(sp_thin_info_tb, file = "./data/processed/cleaning-summaries/sp_thin_info_tb.csv", append = TRUE, col.names = !file.exists("./data/processed/cleaning-summaries/sp_thin_info_tb.csv"))
    ...
    sp_thin_too_big_tb <- data.frame(name = accepted_name_v, 
                                     tooBig = TRUE, 
                                     numOfRecords = nrow(occur_data))
    fwrite(sp_thin_too_big_tb, file = "./data/processed/cleaning-summaries/sp_thin_too_big.csv", append = TRUE, col.names = !file.exists("./data/processed/cleaning-summaries/spThin_too_big.csv"))
  }
print(part)
}, error = function(e) {
  # Log any errors
