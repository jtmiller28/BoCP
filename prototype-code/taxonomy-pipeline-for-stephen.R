### Aligning Taxonomy between POWO/WCVP and NCBI for backfilling genetic info
### Author: JT Miller
...
#### Filter down data for Stephen's Trees
final_table_viburnum_genus <- output_df %>% 
  distinct() %>% 
  filter(wcvpAlignedGenus == "Viburnum" | ncbiAlignedGenus == "Viburnum" | grepl("Viburnum", name, ignore.case = TRUE)) %>% 
  select(alignedName, alignedParentName, name, source, sourceConflict, mult_ncbi_ids, ncbiAcceptedNameUsageID, ncbiPossibleIDs)

fwrite(final_table_viburnum_genus, "/home/millerjared/blue_guralnick/millerjared/BoCP/outputs/Viburnum-example-alignment-table.csv")
final_table_viburnaceae <- output_df %>% 
  distinct() %>% 
  filter(wcvpAlignedFamily == "Adoxaceae" | wcvpAlignedFamily == "Viburnaceae" | wcvpAlignedGenus == "Viburnum" | grepl("Viburnum", wcvpName, ignore.case = TRUE) |
           ncbiAlignedFamily == "Adoxaceae" | ncbiAlignedFamily == "Viburnaceae" | ncbiAlignedGenus == "Viburnum" | grepl("Viburnum", ncbiName, ignore.case = TRUE)) %>% 
  select(alignedName, alignedParentName, name, source, sourceConflict, mult_ncbi_ids, ncbiAcceptedNameUsageID, ncbiPossibleIDs)
fwrite(final_table_viburnaceae, "/home/millerjared/blue_guralnick/millerjared/BoCP/outputs/Viburnaceae-example-alignment.csv")

final_table_dipsacales <- output_df %>% 
  distinct() %>% 
  filter(wcvpAlignedFamily == "Adoxaceae" | wcvpAlignedFamily == "Viburnaceae" | wcvpAlignedFamily == "Caprifoliaceae"
         | ncbiAlignedOrder == "Dipsacales") %>% 
  mutate(nameStatus = ifelse(alignedName == name, "Accepted", "Synonym"))
fwrite(final_table_dipsacales, "/home/millerjared/blue_guralnick/millerjared/BoCP/outputs/dipsacales-example-alignment-table.csv")
names(final_table_dipsacales)

### Back add higher level taxonomy using known family to order relations for angiosperms and gymnosperms. 
# load in the relations of family to order for angiosperms and gymnosperms
apg_relations <- fread("/home/millerjared/blue_guralnick/millerjared/BoCP/data/raw/APG5.csv")
gymno_relations <- fread("/home/millerjared/blue_guralnick/millerjared/BoCP/data/raw/gymnosperms-fams-to-orders.csv")
# merge for a seed plant relations table
seed_plant_relations <- rbind(apg_relations, gymno_relations)
# pull out clade of interest 
poales_fams <- seed_plant_relations[order == "Poales"]
final_table_poales <- output_df %>% 
  distinct() %>% 
  filter(wcvpAlignedFamily %in% poales_fams$family | ncbiAlignedOrder == "Poales") %>% 
  mutate(nameStatus = ifelse(alignedName == name, "Accepted", "Synonym"))
fwrite(final_table_poales, "/home/millerjared/blue_guralnick/millerjared/BoCP/outputs/Poales-example-alignment.csv")
