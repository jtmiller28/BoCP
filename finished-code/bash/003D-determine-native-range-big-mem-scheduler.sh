...
#SBATCH --job-name=determine-native-range-big-mem-array               # Job name
...
#SBATCH --output=/blue/guralnick/millerjared/BoCP/logs/native-range-big-mem-array_%A-%a.out                 # Standard output and error log
#SBATCH --nodes=1                        # Run all processes on a single node
#SBATCH --ntasks=1                       # Run a single task
#SBATCH --cpus-per-task=1               # Number of CPU cores per task
#SBATCH --mem-per-cpu=25gb                   # Job memory request
#SBATCH --time=00-24:00:00               # Time limit days-hrs:min:sec
#SBATCH --array=1-33                  # Array Range
...
Rscript --vanilla /blue/guralnick/millerjared/BoCP/finished-code/r/003D-determine-native-range-big-mem.R

done

date
## example for running: sbatch /blue/guralnick/millerjared/BoCP/finished-code/bash/003D-determine-native-range-big-mem-scheduler.sh