...
### additionally creating subsets of species to model for representation
# load libraries
library(data.table)
library(tidyverse)

# load data
sp_occ <- fread("/blue/guralnick/millerjared/BoCP/outputs/5km_cells_occupied_and_convex_area.csv")
...
sp_model_occs <- sp_occ %>% 
  filter(!is.na(num_5km_cells)) %>% 
  filter(num_5km_cells > 10) %>% 
  mutate(convex_hull_area = ifelse(convex_hull_area == 0, NA, convex_hull_area))
...
# build a histogram for area of convex hulls
ggplot(sp_model_occs, mapping = aes(x = convex_hull_area)) + 
  geom_histogram(aes(y = ..density..), binwidth = 200, fill = "goldenrod", color = "black") +
  geom_density(alpha = 0.5, fill = "darkblue")

# remove convex hulls that failed to process...
sp_model_occs <- sp_model_occs %>% 
  filter(!is.na(convex_hull_area))
# now bin into quantiles of data quantity. 
sp_model_occs$hull_size_percentile <- cut(sp_model_occs$convex_hull_area, 
                                         breaks = quantile(sp_model_occs$convex_hull_area, probs = seq(0,1,0.25), na.rm = TRUE), 
                                         include.lowest = TRUE, 
                                         labels = c("0-25th", "26-50th", "51-75th", "76-100th"))

# Take random sample of species
set.seed(782)
selected_0_25 <- sp_model_occs %>% filter(hull_size_percentile == "0-25th") %>% slice_sample(n = 25)
selected_26_50 <- sp_model_occs %>% filter(hull_size_percentile == "26-50th") %>% slice_sample(n = 25)
selected_51_75 <- sp_model_occs %>% filter(hull_size_percentile == "51-75th") %>% slice_sample(n = 25)
selected_76_100 <- sp_model_occs %>% filter(hull_size_percentile == "76-100th") %>% slice_sample(n = 25)

selected_sp <- bind_rows(selected_0_25, selected_26_50, selected_51_75, selected_76_100)
fwrite(selected_sp, "/blue/guralnick/millerjared/BoCP/outputs/select-sp-test-table.csv")
