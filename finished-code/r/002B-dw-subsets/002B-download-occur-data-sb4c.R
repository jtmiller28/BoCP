...
accepted_name_v <- accepted_name_v[6501:8500]
accepted_name_v <- accepted_name_v[1251:1500]
...
retry_download <- function(i, retry_count) {
  if (retry_count >= 20) {
    ...
    fwrite(failed_names_df, "./data/gbif-dws/raw-dw-info-tbls/failed_names4.csv", append = TRUE, col.names = FALSE)
    return(NULL)
  }
...
for (j in 1:length(accepted_name_v)) {
  ...
  if (!is.null(occurrence_data)) {
    ...
    fwrite(info_df_temp, file = "./data/gbif-dws/raw-dw-info-tbls/raw-info-tbl4.csv", append = TRUE, col.names = !file.exists("./data/gbif-dws/raw-dw-info-tbls/raw-info-tbl4.csv"))
