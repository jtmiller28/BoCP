...
# Init starting of accepted_name_v depending on starting point. (remove for rerun.)
accepted_name_v <- accepted_name_v[15295:32867]
accepted_name_filestyle_v <- accepted_name_filestyle_v[15295:32867]
...
for(i in 1:length(accepted_name_v)){
  ...
  if(any(name_relations$acceptedName == name_relations$acceptedNameParent)){
  accepted_name_relation <- name_relations %>% filter(acceptedName == acceptedNameParent)
  } else {
    accepted_name_relation <- name_relations %>% distinct(acceptedName, .keep_all = TRUE)
  }
  ...
  occurrence_only_acceptedName_data <- occurrence_name_filtered[scientificNameParsed %in% accepted_name] 
  ...
  if(any(name_relations$acceptedNameMultMapsPossible) == TRUE){
    ...
    if(length(unique(accepted_name_relation_less$acceptedNameScientificNameAuthorship)) == 1){
    acceptedAuthorship <- unique(accepted_name_relation_less$acceptedNameScientificNameAuthorship)
    } else{ # a required statement as it seems there is a corner case that trips filtering...
      acceptedAuthorship <- unique(accepted_name_relation_less$acceptedNameScientificNameAuthorship)[1] # take the first one...
    }
  ...
  } # end of if raw_occur_data is null
} # end of i loop
