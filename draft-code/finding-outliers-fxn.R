...
dir.create(paste0("./outputs/testing-outlier-removal/", accepted_name_filestyle, "/all-data/100km"))
dir.create(paste0("./outputs/testing-outlier-removal/", accepted_name_filestyle, "/all-data/250km"))
dir.create(paste0("./outputs/testing-outlier-removal/", accepted_name_filestyle, "/all-data/500km"))
dir.create(paste0("./outputs/testing-outlier-removal/", accepted_name_filestyle, "/native-data"))
dir.create(paste0("./outputs/testing-outlier-removal/", accepted_name_filestyle, "/native-data/100km"))
dir.create(paste0("./outputs/testing-outlier-removal/", accepted_name_filestyle, "/native-data/250km"))
dir.create(paste0("./outputs/testing-outlier-removal/", accepted_name_filestyle, "/native-data/500km"))
dir.create(paste0("./outputs/testing-outlier-removal/", accepted_name_filestyle, "/native-plus-introduced-data"))
dir.create(paste0("./outputs/testing-outlier-removal/", accepted_name_filestyle, "/native-plus-introduced-data/100km"))
dir.create(paste0("./outputs/testing-outlier-removal/", accepted_name_filestyle, "/native-plus-introduced-data/250km"))
dir.create(paste0("./outputs/testing-outlier-removal/", accepted_name_filestyle, "/native-plus-introduced-data/500km"))
...
outlier_analysis <- function(our_thin, our_tdi, accepted_name, accepted_name_filestyle, 
                             what_data, # can be native, all, or introduced
                             input_dir = "./data/processed/regioned-data/",
                             output_dir = "./outputs/testing-outlier-removal/") {
  
  # Check for input file
  file_path <- file.path(paste0(input_dir, what_data, "/", accepted_name_filestyle, ".csv"))
  ...
  # Load data
  occur_data <- fread(file_path)
  
    # Create dir for thinning file
    # dir.create(paste0(output_dir, accepted_name_filestyle, "-", what_data, "-minDist-", our_tdi, "km", 
    #                      "-thinning-", our_thin, "km"))
    output_thin_dir<- paste0(output_dir, accepted_name_filestyle, "/", accepted_name_filestyle, "-", what_data, "-minDist-", our_tdi, "km", 
                                 "-thinning-", our_thin, "km")
    
    # Apply thinning
    spThin::thin(
      occur_data, lat.col = "roundedLatitude", long.col = "roundedLongitude", 
      spec.col = "parentTaxon", reps = 1, write.files = TRUE, 
      out.base = "thinned_data",
      #out.dir = file.path(output_thin_dir, paste0(accepted_name_filestyle, "-", what_data, "-minDist-", our_tdi, "km", "-thinning-", our_thin, "km")),
      out.dir = file.path(paste0(output_dir, accepted_name_filestyle, "/", accepted_name_filestyle, "-", what_data, "-minDist-", our_tdi, "km", 
                                 "-thinning-", our_thin, "km")),
      thin.par = our_thin
    )
    
    # Reload the thinned data
   # thinned_path <- #file.path(output_thin_dir, paste0(accepted_name_filestyle, "-", what_data, "-minDist-", our_tdi, "km", "-thinning-", our_thin, "km/thinned_data_thin1.csv"))
    occur_data <- fread(paste0(output_thin_dir, "/thinned_data_thin1.csv")) %>%
      mutate(id = 1:n())

    # Perform outlier detection
    id_dist <- CoordinateCleaner::cc_outl(occur_data, lon = "roundedLongitude", lat = "roundedLatitude", species = "parentTaxon",
                                          method = "distance", mltpl = 5, tdi = our_tdi, value = "flagged", sampling_thresh = 0, 
                                          verbose = TRUE, min_occs = 7)
    
    id_mad <- CoordinateCleaner::cc_outl(occur_data, lon = "roundedLongitude", lat = "roundedLatitude", species = "parentTaxon", 
                                         method = "mad", mltpl = 5,tdi = our_tdi, value = "flagged",sampling_thresh = 0,
                                         verbose = TRUE, min_occs = 7)
    # 
    # id_quantile <- CoordinateCleaner::cc_outl(occur_data, lon = "roundedLongitude", lat = "roundedLatitude", species = "parentTaxon", 
    #                                           method = "quantile", mltpl = 5,tdi = our_tdi, value = "flagged",sampling_thresh = 0,
    #                                           verbose = TRUE, min_occs = 7)
    # Invert flag values
    id_dist <- !id_dist 
    id_mad <- !id_mad
    ...
    # Generate convex hull and bounding box
    hull <- st_convex_hull(st_union(occur_data_sf))
    hull_bbox <- st_bbox(hull)
    
    # Conduct some calculations for the tests used. 
    # for dist method
    ...
    dist_plot <- ggplot() +
      ...
      theme_minimal() +
      ggtitle(paste0( what_data, "Distribution for ", accepted_name, " of ", quantile_id, " quantile", "\n using Distance Method with", our_tdi, "km outlier threshold", "\n and ",our_thin, "km thinning"))
    
    # Save plot
    ggsave(
      filename = file.path(output_dir, paste0(accepted_name_filestyle, "/", what_data, "-data/", "/", our_tdi, "km/", accepted_name_filestyle, "_dist_plot_", our_tdi, "km-minDist-", our_thin, "km-thin.jpg")),
      plot = dist_plot, height = 10, width = 10
    )
    ...
    mad_hist_plot <- ggplot(data = data.frame(min_distance = mins), aes(x = min_distance)) +
      ...
      labs(title = paste("Histogram of Minimum Distances for", accepted_name, "Mean Absolute Distance Method with ", our_tdi,  "\n and ", our_thin, "km thinning"),
    ...
    # Save plot
    ggsave(
      filename = file.path(output_dir, paste0(accepted_name_filestyle, "/", what_data, "-data/", "/", our_tdi, "km/", accepted_name_filestyle, "_mad_plot_", our_tdi, "km-minDist-", our_thin, "km-thin.jpg")),
      plot = mad_plot, height = 10, width = 10
    )
    ggsave(
      filename = file.path(output_dir, paste0(accepted_name_filestyle, "/", what_data, "-data/", "/", our_tdi, "km/", accepted_name_filestyle, "_mad_hist_plot_", our_tdi, "km-minDist-", our_thin, "km-thin.jpg")),
      plot = mad_hist_plot, height = 10, width = 10
    )
    
    # Wrap up a pdf output with all of these plots
    # create a pdf output
    pdf(file = paste0("./outputs/outlier-pdfs/", what_data, "-data/", our_tdi, "km/", accepted_name_filestyle, "-w-" ,our_thin,"km-thin", "plots.pdf"),  width = 30, height = 30)
    grid.arrange(dist_plot, mad_plot, mad_hist_plot, ncol = 1)
    dev.off()
  
}

# native
outlier_analysis(our_thin = 1, our_tdi = 100, accepted_name = accepted_name, accepted_name_filestyle = accepted_name_filestyle, what_data = "native")
outlier_analysis(our_thin = 5, our_tdi = 100, accepted_name = accepted_name, accepted_name_filestyle = accepted_name_filestyle, what_data = "native")
outlier_analysis(our_thin = 10, our_tdi = 100, accepted_name = accepted_name, accepted_name_filestyle = accepted_name_filestyle, what_data = "native")

outlier_analysis(our_thin = 1, our_tdi = 250, accepted_name = accepted_name, accepted_name_filestyle = accepted_name_filestyle, what_data = "native")
outlier_analysis(our_thin = 5, our_tdi = 250, accepted_name = accepted_name, accepted_name_filestyle = accepted_name_filestyle, what_data = "native")
outlier_analysis(our_thin = 10, our_tdi = 250, accepted_name = accepted_name, accepted_name_filestyle = accepted_name_filestyle, what_data = "native")

outlier_analysis(our_thin = 1, our_tdi = 500, accepted_name = accepted_name, accepted_name_filestyle = accepted_name_filestyle, what_data = "native")
outlier_analysis(our_thin = 5, our_tdi = 500, accepted_name = accepted_name, accepted_name_filestyle = accepted_name_filestyle, what_data = "native")
outlier_analysis(our_thin = 10, our_tdi = 500, accepted_name = accepted_name, accepted_name_filestyle = accepted_name_filestyle, what_data = "native")

# all
outlier_analysis(our_thin = 1, our_tdi = 100, accepted_name = accepted_name, accepted_name_filestyle = accepted_name_filestyle, what_data = "all")
outlier_analysis(our_thin = 5, our_tdi = 100, accepted_name = accepted_name, accepted_name_filestyle = accepted_name_filestyle, what_data = "all")
outlier_analysis(our_thin = 10, our_tdi = 100, accepted_name = accepted_name, accepted_name_filestyle = accepted_name_filestyle, what_data = "all")

outlier_analysis(our_thin = 1, our_tdi = 250, accepted_name = accepted_name, accepted_name_filestyle = accepted_name_filestyle, what_data = "all")
outlier_analysis(our_thin = 5, our_tdi = 250, accepted_name = accepted_name, accepted_name_filestyle = accepted_name_filestyle, what_data = "all")
outlier_analysis(our_thin = 10, our_tdi = 250, accepted_name = accepted_name, accepted_name_filestyle = accepted_name_filestyle, what_data = "all")

outlier_analysis(our_thin = 1, our_tdi = 500, accepted_name = accepted_name, accepted_name_filestyle = accepted_name_filestyle, what_data = "all")
outlier_analysis(our_thin = 5, our_tdi = 500, accepted_name = accepted_name, accepted_name_filestyle = accepted_name_filestyle, what_data = "all")
outlier_analysis(our_thin = 10, our_tdi = 500, accepted_name = accepted_name, accepted_name_filestyle = accepted_name_filestyle, what_data = "all")

# introduced
outlier_analysis(our_thin = 1, our_tdi = 100, accepted_name = accepted_name, accepted_name_filestyle = accepted_name_filestyle, what_data = "introduced")
outlier_analysis(our_thin = 5, our_tdi = 100, accepted_name = accepted_name, accepted_name_filestyle = accepted_name_filestyle, what_data = "introduced")
outlier_analysis(our_thin = 10, our_tdi = 100, accepted_name = accepted_name, accepted_name_filestyle = accepted_name_filestyle, what_data = "introduced")

outlier_analysis(our_thin = 1, our_tdi = 250, accepted_name = accepted_name, accepted_name_filestyle = accepted_name_filestyle, what_data = "introduced")
outlier_analysis(our_thin = 5, our_tdi = 250, accepted_name = accepted_name, accepted_name_filestyle = accepted_name_filestyle, what_data = "introduced")
outlier_analysis(our_thin = 10, our_tdi = 250, accepted_name = accepted_name, accepted_name_filestyle = accepted_name_filestyle, what_data = "introduced")

outlier_analysis(our_thin = 1, our_tdi = 500, accepted_name = accepted_name, accepted_name_filestyle = accepted_name_filestyle, what_data = "introduced")
outlier_analysis(our_thin = 5, our_tdi = 500, accepted_name = accepted_name, accepted_name_filestyle = accepted_name_filestyle, what_data = "introduced")
outlier_analysis(our_thin = 10, our_tdi = 500, accepted_name = accepted_name, accepted_name_filestyle = accepted_name_filestyle, what_data = "introduced")