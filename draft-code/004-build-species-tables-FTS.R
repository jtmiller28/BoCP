### A remake of the build species tables step, trying to use Full Text Search to optimize the fuzzy matches. 
library(DBI)
library(RSQLite)
...
list_found_names <- list.files("/blue/guralnick/millerjared/BoCP/data/processed/species-occs-optim/")
...
# Combine all names into a single regex pattern separated by |
names_pattern <- paste0("(", paste0(names_to_retrieve, collapse = "|"), ")")

# Add wildcards for SIMILAR TO clause
regex_query <- paste0("'%", names_pattern, "%'")

# Construct the SQL query using SIMILAR TO (case-insensitive via UPPER)
query_idigbio <- paste0("SELECT * FROM '", idigbio_file_path, 
                        "' WHERE UPPER(verbatimScientificName) SIMILAR TO ", regex_query)

query_gbif <- paste0("SELECT * FROM '", gbif_file_path, 
                     "' WHERE UPPER(verbatimScientificName) SIMILAR TO ", regex_query)

query_symbiota <- paste0("SELECT * FROM '", symbiota_file_path, 
                         "' WHERE UPPER(verbatimScientificName) SIMILAR TO ", regex_query)
